...
mod value {
    ...
    struct OnDiskStruct {
        term: u64,
        voted_for: Option<sorock::ServerAddress>,
    }
...
impl BallotStore {
    pub fn new(db: Arc<Database>, shard_id: u32) -> Result<Self> {
        ...
        let tx = db.begin_write()?;
        {
            ...
            if tbl.get(shard_id)?.is_none() {
                tbl.insert(shard_id, value::ser(Ballot::new()))?;
            }
        }
        tx.commit()?;
    ...
    pub async fn save_ballot(&self, ballot: Ballot) -> Result<()> {
        ...
        {
            let mut tbl = tx.open_table(table_def(BALLOT))?;
            tbl.insert(self.shard_id, value::ser(ballot))?;
        }
        tx.commit()?;
        Ok(())
    }
